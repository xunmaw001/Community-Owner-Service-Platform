    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="废品类型" prop="feipinleixing">
          <el-select v-model="ruleForm.feipinleixing" placeholder="请选择废品类型">
            <el-option
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.feipintupian" label="废品图片" prop="feipintupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.feipintupian.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
	...
	this.ruleForm.feipinxiangxi = this.ruleForm.feipinxiangxi.replace(reg,'../../../ssmfado9/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `feipinxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.feipinxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
